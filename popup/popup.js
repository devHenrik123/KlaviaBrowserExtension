( async () => {
    ...
    async function reloadKlavia() {
        ...
        for (const tab of tabs) {
            try {
                ...
                if (
                    tab.id &&
                    variousUtils.KlaviaDomains.some(domain => url.hostname.includes(domain))
                ) {
    ...
    document.getElementById("buttonReloadKlavia").addEventListener("click", reloadKlavia);

    // Load settings manually, in case we're slower than the DOMContentLoaded event.
    loadSettings();
})();
